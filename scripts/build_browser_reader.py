...
// button. The multi-GB model fetch is behind an explicit press -- a page a
...
function boot() {{
  ...
  status("Ready. The first press downloads the model into browser cache "
       + "(about 2.4 GB, once — it stays for next time), then reads the "
       + "page right here on your graphics card.");
  go.disabled = false;
}}
...
go.addEventListener("click", async () => {{
  ...
  if (!engine) {{
    try {{
      ...
      status("The model could not load here: " + String(e).slice(0, 120));
    }}
